...
def listar_alumnos():
    try:
        ...
        return jsonify({"mensaje": "Error BD: {}".format(ex), 'exito': False}), 500

def pagina_no_encontrada(error):
    return jsonify({"mensaje": "Error 404: {}".format(error), 'exito': False}), 404
...
def leer_alumno(mat):
    try:
        ...
        return jsonify({'mensaje': "Error", 'exito': False}), 500
    ...
    try:
        ...
        if alumno:
            ...
            sql = """INSERT INTO alumnos (matricula, nombre, apaterno, amaterno, correo)
                    VALUES (%s, %s, %s, %s, %s)"""
            datos = (request.json['matricula'], request.json['nombre'], request.json['apaterno'], request.json['amaterno'], request.json['correo'])
            cursor.execute(sql, datos)
            conexion.connection.commit()  # Confirma la acción de inserción.
            cursor.close()
            return jsonify({'mensaje': "Alumno registrado.", 'exito': True})
    except Exception as ex:
        return jsonify({'mensaje': "Error: {}".format(ex), 'exito': False}), 500
